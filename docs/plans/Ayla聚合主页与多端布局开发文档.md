...
F4 一级直播 tab + 直播间切换（窄屏：底栏下滑走+输入框滑入、上下滑切全部可见；宽屏：按钮+键盘、弹幕侧列）；群内直播子界面（范围=仅该群）
F5 一级语音 tab + 语音房输入框；群内语音子界面
F6 posts 全套（信息流、详情评论、FAB 发帖、群内输入框发帖、个人页我的帖子）
F7 桌游房间框架（列表、创建、占位界面）
F8 消息中心（私信 + 好友/群申请）+ badges 红点
F9 全局搜索页
F10 个人界面扩展、收藏页占位、更多菜单
```
