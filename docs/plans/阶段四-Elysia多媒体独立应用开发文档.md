...
- **M4-3 媒体**：对象存储上传 + 缩略图/波形 + 表情包 + 文件。✅ 已完成（契约层，见 [阶段四-M4-3媒体开发步骤.md](./阶段四-M4-3媒体开发步骤.md)；本地 MinIO 端到端冒烟与爱莉媒体投影真实 E2E 未验收，如实标注）
- **M4-4 爱莉桥接**：service credential 获取 + inject 入站 + SSE 出站投影 + 爱莉用户身份 + `elysia.reply` 事件。✅ 已完成并真实端到端验收（见 [阶段四-M4-4爱莉桥接开发步骤.md](./阶段四-M4-4爱莉桥接开发步骤.md)；2026-08-12 真实 Elysium + credential + Ayla 前端 Playwright 收发闭环与刷新持久化通过）
- **M4-5 语音**：LiveKit 集成 + 语音频道 + token 签发 + 爱莉 Voice Live 接入。✅ 已完成（契约层，见 [阶段四-M4-5语音开发步骤.md](./阶段四-M4-5语音开发步骤.md)；2026-08-13 契约测试全绿 + 爱莉桥接编排端点/事件分派/poll 投影补齐；真实 LiveKit/爱莉入会闭环未验收，如实标注。已知公共契约缺口：阶段三 `voice_call.*` SSE 事件流未进 Elysium Life Event 账本，转写投影走 transcripts 轮询绕开，未改动 Elysium 侧代码）
- **M4-6 直播**：SRS 集成 + 直播间 + 弹幕 + 爱莉直播订阅。✅ 已完成（见 [阶段四-M4-6直播开发步骤.md](./阶段四-M4-6直播开发步骤.md)；契约测试 30 passed + 真实冒烟通过，爱莉直播对接不验收，如实标注）
- **M4-7 桌游**：狼人杀房间 + 玩家视图 + AI 玩家 + 复盘。

### 阶段五（React 前端）
- **M5-1 基座**：Vite + React + TS + Zustand + 路由 + 登录注册 + 全局状态。✅ 已完成（见 [阶段五-M5-1前端基座开发步骤.md](./阶段五-M5-1前端基座开发步骤.md)）
- **M5-2 聊天界面**：会话列表 + 聊天窗口 + 消息渲染（文本/图片/语音/文件/表情）+ 已读/撤回。✅ 已完成（界面架子；见 [阶段五-M5-2聊天界面开发步骤.md](./阶段五-M5-2聊天界面开发步骤.md)；媒体消息渲染占位，端到端待真实 Redis/Elysium 验收）
- **M5-3 语音界面**：语音频道 + 通话控制（静音/音量）。✅ 已完成（契约层；见 [阶段五-M5-3语音界面开发步骤.md](./阶段五-M5-3语音界面开发步骤.md)；44 个契约测试全绿，真实 LiveKit/Elysium E2E 未验收）
- **M5-4 直播界面**：直播间 + 弹幕 + 播放器。✅ 已完成（契约层；见 [阶段五-M5-4直播界面开发步骤.md](./阶段五-M5-4直播界面开发步骤.md)；30 个契约测试全绿 + build 通过，真实 SRS/E2E 未验收，如实标注）
