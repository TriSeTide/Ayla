...
- 搜索提供“全部、用户、群聊、帖子、直播间、桌游室”六个筛选。全部保留原五类分组和各组独立续页，类型筛选仅显示对应类别。查询、分类、账号共同隔离缓存与滚动位置；切换新分类从顶部开始，返回已浏览分类恢复原位置，旧请求不得落到新分类。
- 当前搜索页通过宽/窄顶栏提交或清空关键词时保留有效分类；从其他页面新发起搜索默认全部。顶栏在搜索页随URL关键词更新，只有分类变化时不覆盖尚未提交的输入。
- 搜索与收藏共用`DirectoryFilters`：>768px为176px左栏，页面12px外沿、筛选与结果之间12px间隔，结果轨道填满余宽；≤768px为结果区上方固定筛选，搜索三列两行、收藏四列两行。筛选处于结果滚动容器之外，滚动结果不能带走筛选或上方导航。筛选支持方向键、Home/End、选中态与tabpanel关联，减少动态时直接归位。
- 全部结果在>1024px保留两列类别布局，窄于该宽度为单列；单个类型始终单列。历史、加载、错误、空状态共用结果轨道。用户结果姓名/签名上下排并可省略；群结果使用36px `Avatar` 展示既有搜索接口返回的群头像，资源为空或失败时使用组件原有回退，不从聊天store补造结果。内部图片仍走鉴权签名路径，群头像公开边界仅限被可发现群完整引用的图片；私聊头像与普通附件保留原权限，详见媒体架构文档。
...
- 收藏筛选按§12.9使用宽屏左栏/窄屏固定顶部，保留全部、消息、帖子、直播间、语音房、桌游室、群聊七类。结果区是独立滚动容器，分类切换与详情返回保存该容器的位置。769–1024px结果保持单列，避免新增左栏挤窄卡片；>1024px使用与帖子相同的`useMasonryColumns`和两条flex列，列间距12px。结果填满可用余宽，不再限制1200px并居中留白；标题横跨筛选与结果。
