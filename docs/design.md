...
- 个人页与公开的他人页在>768px使用同一两栏布局：顶栏横跨，左侧资料/设置，右侧本人或公开内容。有右侧内容列时（本人页恒有；他人页开启「向他人展示内容」时）为**侧栏模式**（照搬语音列表界面 directory-page 布局：页面根 overflow hidden + 中间层不裁剪 + 侧栏 stretch 自滚动 + 内容列独立滚动）：页面不滚动，左侧资料卡是占满高度的固定侧栏（底部呼吸由自身 margin 承担，内容超出时内部滚动兜底），右侧内容列独立滚动（OverlayScrollbar 自动接管，滚动裁切线延伸到视口物理底边）。侧栏资料卡区块间距放宽、表单区铺满中间、操作区沉底。无公开内容、加载或错误分支回到居中单列，页面整页滚动、卡片自然高度；窄屏wrapper为 `display:contents` 保持原顺序与间距。
- 个人页左栏280–340px、两栏卡片净间隔16px、页面外沿24px（侧栏模式底部0），主体两栏不设1200px上限。侧栏资料卡内沿16px、卡内区块间距24px，表单字段间距16px；右侧内容列滚动区左右8px、底部24px呼吸间距。
