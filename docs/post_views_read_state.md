...
- 群内帖子首屏只请求20条，滚到底后使用保留的`next_cursor`再请求一页；不再为了覆盖未读而首屏连续拉10页。未加载的未读数仍来自会话的`post_unread_count`，不能用本页条数代替全群未读总数。
- 未读帖子 = 列表数据中 `is_viewed=false` 的；滚动容器上方/下方有未读时显示
  「↑/↓ N 条未读帖子」胶囊标签（复用聊天界面 `message-jump-tags` 语言与样式），点击滚动到最近的已加载未读帖子；下方未读尚未加载时请求下一页，不把未加载误判为已读。加载失败保留旧卡与cursor，明确重试。
- 每看到一条（进入视口）即上报已读，标签与红点实时减少；视口内全部看完后标签消失。

## 数据流

- 一级帖子信息流与群内列表均以20条为一页，持有自己已经加载的页和cursor；全局帖子store仍负责共享浏览/收藏/WS投影，不能据其中某一页缓存推断完整群目录。
- 一级信息流刷新使用请求revision压过旧追加。后到的HTTP快照不能倒退当前已读/浏览量，也不能复活请求期间已收到删除事件的帖子；刷新期间WS新建帖继续保留。下一页失败保留旧页与cursor并显示明确重试，详情返回保留页与滚动，仅后续新增DOM播放入场。
