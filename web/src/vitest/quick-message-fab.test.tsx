 ...
 *
 * - 初始展开，点击通过 shell store 打开快捷消息栏（不渲染 QuickMessagesSheet）；
...
describe("QuickMessageFab 半贴交互", () => {
  it("初始展开，点击打开快捷消息栏（置 shell store）", () => {
    ...
    expect(useShellStore.getState().quickMessagesOpen).toBe(true);
  });
  ...
  it("4s 无点击自动半贴，点一下展开（不打开快捷栏）", () => {
    ...
    expect(screen.getByRole("button", { name: "消息，2 条未读" })).toBeInTheDocument();
    expect(useShellStore.getState().quickMessagesOpen).toBe(false);
  });
