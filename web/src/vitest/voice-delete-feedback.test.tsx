...
describe("current voice room delete failure", () => {
  it.each([false, true])("keeps the room and exposes the failed delete in the %s group variant", async (group) => {
    ...
    await waitFor(() => expect(screen.getByRole("alert")).toHaveTextContent("删除被拒绝"));
