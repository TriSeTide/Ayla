...
function mockRooms(rooms: GameRoom[]) {
  vi.mocked(boardgameApi.listGameRoomsPage).mockImplementation(async (params) => {
    const results = rooms.filter((item) => !params?.groupId || (item.allowed_group_ids ?? []).includes(params.groupId));
...
describe("GroupGames 创建后刷新", () => {
  it("收到 room-created 事件后重新加载并显示新房间", async () => {
    ...
    await waitFor(() => expect(boardgameApi.listGameRoomsPage).toHaveBeenCalledTimes(1));
    ...
    mockRooms([room("新房间", "group-1")]);
    window.dispatchEvent(new CustomEvent("boardgame:room-created", { detail: room("新房间", "group-1") }));

    await waitFor(() => expect(boardgameApi.listGameRoomsPage).toHaveBeenCalledTimes(2));
    ...
    expect(boardgameApi.listGameRoomsPage).toHaveBeenCalledWith({ groupId: "group-1", onlyLive: undefined, limit: 20, cursor: null });
    expect(animatedCards.map((item) => item.node)).toEqual(Array.from(cards));
    expect(animatedCards.map((item) => item.delay)).toEqual([0]);
  });
});
...
describe("GroupGames 多群可见性（allowed_group_ids）", () => {
  it("同一多群房间（group=null + 白名单 13/14/15）出现在每个被选群的群内页", async () => {
    ...
    for (const gid of ["13", "14", "15"]) {
      ...
      expect(boardgameApi.listGameRoomsPage).toHaveBeenCalledWith({ groupId: gid, onlyLive: undefined, limit: 20, cursor: null });
      unmount();
    }
  });
  ...
  it("先看别的群、再看白名单群，多群房间仍在（store 不被单群 scope 覆盖）", async () => {
    // 不同群由独立 groupId 查询取得分页目录，白名单按服务端语义过滤。
